...
════════════════════════════════════════════════════════════════════════════════════════════════════════════════════════
  {__version__} 
  {__guns__}                     {Fore.LIGHTCYAN_EX}Connected as {requests.get(sys_url,headers=s_sys_h()).json()['username']}{Ly}#{Fore.LIGHTCYAN_EX}{requests.get(sys_url,headers=s_sys_h()).json()['discriminator']}{Ly}
...
def checkavail(): 
   if len(available_usernames) < 1:
      print(f"{Lb}[!]{Fore.RED} Sorry: No available usernames found.")
      exit()
   else:
      return
def opt2load():
...
def ch_send_webhook(val0:str):
   if webhook_0 == True:
       ...
       embeds=[
    {
      ...
      "footer": {
        "text": "github.com/lawiis/lrp.git"
      },
      "author": {
        "name": "DSV - Username Found",
        "url": "https://github.com/lawiis/lrp.git",
